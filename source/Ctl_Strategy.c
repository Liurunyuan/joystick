...
#pragma CODE_SECTION(BounceBackFront, "ramfuncs")
void BounceBackFront(void){
    ...
    mass = 100 / (gConfigPara.naturalVibrationFreq * gConfigPara.naturalVibrationFreq);
    ...
    force_openLoop = velocity_force + inertial_force;
    force_closeLoop = force_PidOutput(force_openLoop, gExternalForceState.value);
    force_closeLoop = -force_closeLoop;

    velocity_openLoop = gSysInfo.velocity_last + ((gExternalForceState.value - velocity_force) / mass) * (0.25/1000);
    gSysInfo.velocity_last = gSysInfo.velocity_last + ((gExternalForceState.value - velocity_force) / mass) * (0.25/1000);
    ...
    gSysInfo.targetDuty_V = (int16)((gPidPara.K_V_ODE * velocity_openLoop + B_V) + velocity_closeLoop);
    gSysInfo.targetDuty_F = (int16)((gPidPara.K_F_ODE * force_openLoop + B_F) + force_closeLoop);
    if(((gSysInfo.lastStickDisSection == 4) && (gStickState.value < -0.22)) || ((gSysInfo.lastStickDisSection == 3) && (gStickState.value > 0.22))){
