...
//    }
    ...
    dynamic_friction = - (gExternalForceState.value - spring_force - damp_force - inertial_force);

    if(dynamic_friction < 0){
        abs_dynamic_friction = - dynamic_friction;
    }
    else{
        abs_dynamic_friction = dynamic_friction;
    }

    if(abs_dynamic_friction < friction){
